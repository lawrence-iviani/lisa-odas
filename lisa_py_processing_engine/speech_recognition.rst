LisaSpeechRecognition
=================
A tool to receive speech data and produce transcription and localization of a single speaker among different sources (odas parameter)

Objective (not necessary in order)
- add pipeline mechanism with different processing mechanism (speaker identification, diarization) and finally process in the STT provided by this tool
- add export functionalities to 
- configuration of specific "user" parameters (e.g len of max phrase to detect )
- keyword activation
- tracking of different ODAS input (swtich of the source between channels)
- ROS messages

Run
---
In this order (server, client)
In console 1: (from file location)
python3 lisa_speech_recognition.py

In console 2: (from ./bin)
 ./odaslive -vc ../config/matrix-lisa/matrix_voice_LISA_1.cfg

Credits
-------

Code imported from speech recognition, see below (removed installation from original readme)
