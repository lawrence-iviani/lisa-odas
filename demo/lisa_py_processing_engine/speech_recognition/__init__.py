    ...
    def _metadata_to_str(self):
        ...
        for k, v in self.metadata.items():
            if isinstance(v, tuple):
                s = s + "\n" + str(v[0]) + "\t" + "{}={}".format(k, v[
                    1])  # [kk + ""for kk, vv in v[1].items()])  #"\n\t{}={} {} {}".format(k,v, k.__class__, v.__class__)
                # "".join(["\n\t[s]" + k + "=" + str(v) + "  " for k, v in self.metadata.items()])
            else:
                s = s + "\n\t{}={}".format(k, v)

        return s
        # error return "".join(["\n["+v[0]+"]\t[s]"+k + "=" + str(v[1]) + "  " for k, v in self.metadata.items()])
        # return "".join(["\n\t[s]"+k + "=" + str(v) + "  " for k, v in self.metadata.items()])

    def __str__(self):
        return "{} sr={}Hz, len={}s. Metadata: {}".format(self.__class__, self.sample_rate, self.length,
...
class SpeechListener(AudioSource):
    ...
    def listen(self, source, timeout=None, phrase_time_limit=None, snowboy_configuration=None):
        ...
        while True:
            ...
            while True:
                ...
                if pause_count > pause_buffer_count:  # end of the phrase
                    self.logger.debug(
                        "LISTEN: end of the phrase. pause_count {} > {} ".format(pause_count, pause_buffer_count))
                    break
            ...
            if phrase_count >= phrase_buffer_count or len(buffer) == 0:
                self.logger.debug(
                    "PHRASE COUNT: phrase is long enough or we've reached the end of the stream, so stop listening.")
        ...
        self.logger.debug(
            "Returns from acquisition phrase duration: {} - phrase_count={} pause_count={} phrase_buffer_count={}".format(
                elapsed_time - phrase_start_time, phrase_count, pause_count, phrase_buffer_count))
    ...
    def detect_speech_activity(self, source, detection_callback=None, timeout=None):
        ...
        seconds_per_buffer, pause_buffer_count, phrase_buffer_count, non_speaking_buffer_count = self._detection_constants(
            source)
    ...
    def process_speech(self, source, previous_audio=None, phrase_time_limit=None, detection_callback=None):
        ...
        while True:
            ...
            while True:
                ...
                if energy > self.energy_threshold:
                    ...
                    self.logger.debug("LISTEN: energy > self.energy_threshold, update speech_end: metadata: {}".format(
                        self._audio_metadata_to_str(_metadata_detected_dict)))
                    pause_count = 0
                ...
                if pause_count > pause_buffer_count:  # end of the phrase
                    self.logger.debug("LISTEN: PAUSE DETECTED (pause_count={}), metadata: {}".format(pause_count,
                                                                                                     self._audio_metadata_to_str(
                                                                                                         _metadata_detected_dict)))
                    break
            ...
            self.logger.debug(
                "PHRASE COUNT: phrase_count ({}) >= phrase_buffer_count ({}) or len(buffer) {}==0 ".format(phrase_count,
                                                                                                           ...
                                                                                                           len(buffer)))
            if phrase_count >= phrase_buffer_count or len(buffer) == 0:
                self.logger.debug("PHRASE COUNT: PHRASE END, phrase is long enough. metadata: {}".format(
                    self._audio_metadata_to_str(_metadata_detected_dict)))
                break  # phrase is long enough or we've reached the end of the stream, so stop listening
            else:
                self.logger.debug("PHRASE COUNT: Still frame to decode Next iteration")
        ...
        retval = AudioData(frame_data, source.SAMPLE_RATE, source.SAMPLE_WIDTH)
        # Preserve metadata from previous_audio
        retval.add_metadata(previous_audio.metadata)
        # add metadata extracted at this round
        retval.add_metadata(_metadata_detected_dict)
...
class Recognizer(AudioSource):
    ...
    def listen(self, source, timeout=None, phrase_time_limit=None, snowboy_configuration=None):
        ...
        """
        sl = SpeechListener()
        return sl.listen(source, timeout=timeout, phrase_time_limit=phrase_time_limit,
    ...
    def set_sphinx_params(self, language="en-US", keyword_entries=None, grammar=None):
		...
		"""
        assert isinstance(language, str) or (isinstance(language, tuple) and len(
            language) == 3), "``language`` must be a string or 3-tuple of Sphinx data file paths of the form ``(acoustic_parameters, language_model, phoneme_dictionary)``"
        ...
        self.logger.info("imported pocketsphinx:  {}".format(pocketsphinx))
        # Get the dictonary
        if isinstance(language, str):  # directory containing language data
            ...
            self.logger.debug(
                "recognize_sphinx.get dict: directory containing language data. language {} - language_directory {} ".format(
                    language, language_directory))
        else:  # 3-tuple of Sphinx data file paths
            acoustic_parameters_directory, language_model_file, phoneme_dictionary_file = language
            self.logger.debug(
                "recognize_sphinx.get dict: tuple of Sphinx data file paths. language {} ".format(language))
        ...
        self.logger.info(
            "Identified dictionary_file={}  language language_model_file={} ".format(phoneme_dictionary_file,
                                                                                     language_model_file))

        # create decoder object
        config = pocketsphinx.Decoder.default_config()
        self.logger.debug("recognize_sphinx.create decoder: config={}".format(config))
        ...
        self.logger.debug("recognize_sphinx.create decoder: config={}".format(decoder))

        # obtain recognition results
        if keyword_entries is not None:  # explicitly specified set of keywords
            self.logger.debug("recognize_sphinx.search for keywords: {}".format(keyword_entries))
            ...
            self.logger.debug("recognize_sphinx.search  path to a FSG or JSGF grammar. {}".format(grammar))
            ...
            self.logger.warning("recognize_sphinx.search  no keyword_entries and no grammars")
    ...
    def recognize_sphinx(self, audio_data, show_all=False):
        ...
        _tp = TimeProfiler()
        _tp.start()
        self.logger.debug("sphinx.decode. Start")
        # obtain audio data
        ...
        self.logger.debug("[Exec in {}s]utterance ".format(_tp.add_time('decode utterance')))

        self._decoder.start_utt()  # begin utterance processing
        self.logger.debug("[Exec in {}s]utterance ".format(_tp.add_time('decode utterance')))
        self._decoder.process_raw(raw_data, False,
                                  True)  # process audio data with recognition enabled (no_search = False), as a full utterance (full_utt = True)
        self.logger.debug("[Exec in {}s]process_raw ".format(_tp.add_time('decode process_raw')))
        self._decoder.end_utt()  # stop utterance processing	
        self.logger.debug("[Exec in {}s] sphinx.decode. End  ".format(_tp.add_time('decode end utt')))

        if show_all: return self._decoder
    ...
    def recognize_azure(self, audio_data, key, language="en-US", result_format="simple", profanity="masked",
                        location="westus", show_all=False):
        """
        ...
        access_token, expire_time = getattr(self, "azure_cached_access_token", None), getattr(self,
    ...
    def recognize_bing(self, audio_data, key, language="en-US", show_all=False):
        ...
        access_token, expire_time = getattr(self, "bing_cached_access_token", None), getattr(self,
    ...
    def recognize_lex(self, audio_data, bot_name, bot_alias, user_id, content_type="audio/l16; rate=16000; channels=1",
        ...
        response = client.post_content(botName=bot_name, botAlias=bot_alias, userId=user_id, contentType=content_type,
                                       accept=accept, inputStream=raw_data)
    ...
    def recognize_houndify(self, audio_data, client_id, client_key, show_all=False):
        ...
        wav_data = audio_data.get_wav_data(
            convert_rate=None if audio_data.sample_rate in [8000, 16000] else 16000,
    ...
    def recognize_tensorflow(self, audio_data, tensor_graph='tensorflow-data/conv_actions_frozen.pb',
