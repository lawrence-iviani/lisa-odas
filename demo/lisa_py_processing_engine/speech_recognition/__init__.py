...
class Recognizer(AudioSource):
    ...
    def set_sphinx_params(self, language="en-US", keyword_entries=None, grammar=None):
        ...
        if isinstance(language, str):  # directory containing language data
            ...
            self.logger.debug("recognize_sphinx.get dict: directory containing language data. language {} - language_directory {} ".format(language, language_directory))
        else:  # 3-tuple of Sphinx data file paths
            acoustic_parameters_directory, language_model_file, phoneme_dictionary_file = language
            self.logger.debug("recognize_sphinx.get dict: tuple of Sphinx data file paths. language {} ".format(language))
        ...
        self.logger.info("Identified dictionary_file={}  language language_model_file={} ".format(phoneme_dictionary_file, language_model_file))
    ...
    def recognize_sphinx(self, audio_data, show_all=False):
        ...
        self.logger.debug("[Exec in {}s]utterance ".format(_tp.add_time('decode utterance')))		
        self._decoder.process_raw(raw_data, False, True)  # process audio data with recognition enabled (no_search = False), as a full utterance (full_utt = True)
