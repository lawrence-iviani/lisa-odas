...
def callback_SSL(pSSL_struct):
    ...
    for i in range(0, MAX_ODAS_SOURCES):
        try:
            ...
            # TODO: SAVE THE TIMESTAMP SOMEHOW!!!
            SSL_latest[i] = deepcopy(ssl_str.src[i])
...
def callback_SST(pSST_struct):
    ...
    for i in range(0, MAX_ODAS_SOURCES):
        try:
            ...
            SST_latest[i] = deepcopy(sst_str.src[i])
...
def callback_SSS_S(n_bytes, x):
    ...
    for i in range(0, MAX_ODAS_SOURCES):
        try:
            ...
            logger.warning("SSS_S receiving Queue is Full, skipping frame (TODO: lost for now)")
            # do nothing for now, perhaps extract the first to make space? Kind of circular queue behavior.....
            pass
...
def source_listening(source_id):
    ...
    with sr.OdasRaw(audio_queue=SSS_queue[source_id], sample_rate=SAMPLE_RATE_INCOME_STREAM,
        ...
        while True:  # repeatedly listen for phrases and put the resulting audio on the audio processing job queue
            ...
            logger.info("*-*(source id {})*-* Start Listening ".format(source_id))
            ...
            logger.info("*-*(source id {})*-* Speech DETECTED {}".format(source_id, detected_audio_data))
            if detected_audio_data:
                ...
                logger.info("*-*(source id {})*-* Speech COMPLETED: {}".format(source_id, processed_audio_data))
